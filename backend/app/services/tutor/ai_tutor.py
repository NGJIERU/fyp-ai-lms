...
Response:"""
    ...
    def explain_concept(
        ...
        Args:
            ...
            context: Optional additional context (e.g., related quiz questions)
            
        Returns:
            Explanation with sources
        """
        # Build RAG query - use topic for retrieval, not the full context
        rag_query = topic
        if question:
            rag_query += " " + question
        ...
        # Build the question/context for the prompt
        full_question = question or "No specific question"
        if context:
            full_question = f"{context}\n\nAdditional context: {full_question}" if question else context
