"""Unify materials table for crawled and uploaded content

Revision ID: b6456a87f56d
Revises: 6370c1e0b595
Create Date: 2025-12-12 10:56:50.803575
...
revision: str = 'b6456a87f56d'
down_revision: Union[str, None] = '6370c1e0b595'
...
def upgrade() -> None:
    ...
    op.drop_index(op.f('ix_course_materials_course'), table_name='course_materials')
    ...
    op.drop_index(op.f('ix_course_materials_uploader'), table_name='course_materials')
    op.drop_table('course_materials')
    op.add_column('materials', sa.Column('material_type', sa.String(length=20), nullable=False))
    ...
    op.add_column('materials', sa.Column('download_count', sa.Integer(), nullable=False))
...
def downgrade() -> None:
    ...
    sa.ForeignKeyConstraint(['course_id'], ['courses.id'], name=op.f('course_materials_course_id_fkey'), ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['uploaded_by'], ['users.id'], name=op.f('course_materials_uploaded_by_fkey'), ondelete='SET NULL'),
    sa.PrimaryKeyConstraint('id', name=op.f('course_materials_pkey')),
    sa.UniqueConstraint('file_path', name=op.f('course_materials_file_path_key'), postgresql_include=[], postgresql_nulls_not_distinct=False)
    )
    op.create_index(op.f('ix_course_materials_uploader'), 'course_materials', ['uploaded_by'], unique=False)
    ...
    op.create_index(op.f('ix_course_materials_course'), 'course_materials', ['course_id'], unique=False)
    # ### end Alembic commands ###
